...
for path in SRC.rglob("*.smt2"):
    ...
    # Insert nat_step$ declaration once
    inserted_decl = False
    new_lines = []
    for ln in lines:
        new_lines.append(ln)
    ...
    # Add active Nat$ -> Nat$ assertions before check-sat
    # Use up to first 3 Nat$ vars
    step_asserts = [
    ...
    out.write_text("\n".join(final_lines) + "\n")
    print(f"[NATSTEP {len(step_asserts)} vars] {rel}")
PY
